---
title: "ODHI sysrev"
author: "Gill, Dom, GJ, Rik, Robbert"
...
The Open Science Framework repositor for this project is located at https://osf.io/gbef3/, and the GitHub repo is at https://github.com/Matherion/ehealth-sysrev-of-reviews.
...
queryHitExportPath <- here::here("20-queries");

### Set path for screening
screeningPath <- here::here("10-screening");

### Set path for extraction script template
extractionScriptTemplatePath <- here::here("50-extraction");

### Set path for intermediate output
workingPath <- here::here("70-intermediate-results");

### Set path for figures
figPath <- here::here("80-figures");
...
queries[[length(queries) + 1]] <-
  list(date = ymd("2019-02-01"),
       query = query_full(
         inclusion = query_requiredConcepts(
           conceptName="Ehealth Review Sysrev Query",
           query_conceptTerms(
             conceptName = "Ehealth",
             "ehealth",
             "e-health",
             ...
             "digital health",
             "app"),
           query_conceptTerms(conceptName = "review",
                              "review",
                              "meta-analys*"))),
...
screening_exclusion_criteria <-
  c(noReview = "This article does not describe a review or meta-analysis",
    noEhealth = "This article is not about ehealth/digital health");
...
```{r query1-visual, echo=TRUE, eval=FALSE}
plot(queries[[1]]$query);
```

```{r query1-visual-export, screenshot.force=FALSE, fig.height=5, echo=FALSE}
### Saving file as .png and including it afterwards to show it on
### GitHub preview
forExport <- data.tree::ToDiagrammeRGraph(queries[[1]]$query);
query1export <- file.path(workingPath, "query1.png");
DiagrammeR::export_graph(forExport,
                         query1export);
```

![Figure 1: Conceptual visualisation of query 1.](`r query1export`)
...
```{r query-1-convert-to-interface-languages, echo=TRUE, eval=TRUE}
query_toInterfaceLang(queries[[1]]$query,
                      fields=queries[[1]]$fields,
                      exclude=queries[[1]]$exclude);
```

This query was run at `r queries[[1]]$date;`. The hits were exported from the bibliographic database interface and stored locally in the study repository. They were then imported.

```{r query-1-import-hits, echo=TRUE, eval=FALSE}
...
firstQueryHits_psycinfo <- nrow(firstQueryIteration_psycinfo$records);
...
firstQueryHits_pubmed <- nrow(firstQueryIteration_pubmed$records);

```

The query in PubMed (`r # firstQueryHits_psycinfo` hits; file saved as <code>pubmed-`r queries[[1]]$date`.ris</code>) and PsycINFO accessed through EbscoHost (`r # firstQueryHits_pubmed` hits; file saved as <code>psycinfo-`r queries[[1]]$date`.ris</code>), and exported to RIS format (called 'MEDLINE' in PubMed). The RIS files were then imported in R using `metabefor`.

```{r query-1-merge-hits, echo=TRUE, eval=FALSE}
...
```{r list-screening-criteria, results='asis'}
cat(paste0(paste0("    - **`",
                  names(screening_exclusion_criteria),
                  "`** ",
                  screening_exclusion_criteria,
                  ";\n"),
           collapse=""));
...
sheetsURL <- paste0("https://docs.google.com/spreadsheets/d/",
                    "1fUVtq6mXciJ47i0FufM6mPU67PBWAadwib6h1m93N80");
